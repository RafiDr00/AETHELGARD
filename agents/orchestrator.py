...
class AgentOrchestrator:
    ...
    def __init__(self, knowledge_engine=None, sandbox_executor=None):
        ...
        self._jobs_sync_lock = threading.Lock()
        ...
        self._pipeline_outcomes: Dict[str, Dict[str, Optional[str]]] = {}
    ...
    def create_job(self, scenario: str) -> PipelineJob:
        job_id = f"job-{uuid.uuid4().hex[:12]}"
        job = PipelineJob(job_id=job_id, scenario=scenario)
        with self._jobs_sync_lock:
            self._jobs[job_id] = job
            # Bounded eviction (sync — acceptable because rarely >1000 jobs)
    ...
    def _update_metrics_incremental(self, record: RemediationRecord) -> None:
        """MUST be called inside self._metrics_lock AND self._history_lock."""
        n = self._metrics.total_anomalies_detected
        if n == 0:
            return
    ...
    def get_metrics(self) -> PlatformMetrics:
        # events_processed derives from total_anomalies_detected (already lock-guarded)
        self._metrics.events_processed = self._metrics.total_anomalies_detected * 6
        return self._metrics

    def get_remediation_history(self) -> List[RemediationRecord]:
        # Snapshot copy — safe without lock in CPython (GIL protects list copy)
        return list(self._remediation_history)

    def get_recent_remediations(self, limit: int = 10) -> List[RemediationRecord]:
        # Snapshot slice — safe without lock in CPython
        return list(self._remediation_history[-limit:])

    async def approve_deployment(self, job_id: str) -> None:
